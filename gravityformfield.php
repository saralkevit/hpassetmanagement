...
class Gravity_User_List extends GF_Field_Select {
	...
	public function maybe_add_workflow_field_group( $field_groups ) {
		...
		$field_groups[] = array( 'name' => 'workflow_fields', 'label' => __( 'User Fields', 'gravityforms' ), 'fields' => array() );
		return $field_groups;
	}
	...
	public function get_form_editor_field_title() {
		return __( 'User List', 'gravityflow' );
	}
	...
	public function get_users_as_choices( $value ) {
        ...
        $useriddata = get_user_by( 'email', $value );
        //print_r($useriddata);
        //exit;
        if(isset($useriddata->ID))
        {
            $value = $useriddata->ID;
        }
		$form_id = $this->formId;
...
GF_Fields::register( new Gravity_User_List() );